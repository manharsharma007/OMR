...
{
    public partial class SubUpdateBatch : Form
    {
        data_class data;
        ...
        private void SubUpdateBatch_Load(object sender, EventArgs e)
        {
            DataTable ds = data.select_custom_batch();
...
MessageBoxDefaultButton.Button1);
        ...
        private void add_questions_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ds = data.select_batch_custom_options(Convert.ToInt32(batch_combo_box.SelectedValue));
                da = data.da;
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (saveFileDialog1.ShowDialog() == DialogResult.OK)
                {
                    ...
                    {
                        //get key
                        int topicId = Convert.ToInt32(row.Cells[3].Value);
                        int subTopicId = Convert.ToInt32(row.Cells[4].Value);
                        int number = Convert.ToInt32(row.Cells[1].Value);

                        //avoid updating the last empty row in datagrid

                        row.Cells[3].Value = data.get_topic(topicId, number).ToString();
                        row.Cells[4].Value = data.get_sub_topic(subTopicId, number).ToString();
                    }
