...
{
    public partial class SubAnswerSheet : Form
    {
        ...
        private void SubAnswerSheet_Load(object sender, EventArgs e)
        {
        ...
        private void template_combo_box_SelectedIndexChanged(object sender, EventArgs e)
        {
...
MessageBoxDefaultButton.Button1);
        ...
        {
            _ds_questions = data.select_template_options(id, "Custom");
        ...
        private void add_questions_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if(new_instance)
                {
                    _count = data.count_batch_custom_options(batch_id);
                ...
                {
                    data.insert_batch_custom_option(batch_id, Convert.ToInt32(template_combo_box.SelectedValue), _count + 1, 0, Convert.ToInt32(topic_combo_box.SelectedValue), Convert.ToInt32(sub_topic_combo_box.SelectedValue));
                    _maximum_question--;
                    _count++;
                }

                ds = data.select_batch_custom_options(batch_id);
                da = data.da;
                ...
                dataGridView1.Columns[3].HeaderText = "Topic";
...
MessageBoxDefaultButton.Button1);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (dataGridView1.Rows.Count > 0)
            {
                try
                {
                    string marks = "1,2,3,4,5,0.5";
                    foreach (DataGridViewRow row in dataGridView1.Rows)
                    {
                        row.Cells[2].Value = marks;
                    }
                }
                catch(Exception ex)
                {
